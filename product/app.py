...
with st.sidebar:
    ...
    page = st.radio("Navigate", 
                    ["📤 Analyze Game", "♟ Play Engine", "🧩 Puzzles", "💬 Coach Chat", "📖 Rulebook", "🎓 Rules Chat", "📊 Dashboard"],
...
if page == "📤 Analyze Game":
    ...
    # Check if puzzles are unlocked
    try:
        resp = requests.get(f"{API_URL}/profile/{st.session_state.user_id}", timeout=120)
        if resp.status_code == 200:
    ...
    if games_played < 3:
        st.info(f"🔒 Puzzle recommendations unlock after 3 games analyzed. You've analyzed {games_played}/3 games. Keep going!")
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if relevant and not search_query:
                st.success(f"📌 Based on your weakness (**{weakness.replace('_', ' ')}**), we recommend these entries:")
    ...
    if results and not search_query:
        ...
        for category in sorted(categories):
            ...
            cat_entries = [e for e in results if e["category"] == category]
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if relevant and weakness != "general":
                    st.info(f"💡 Based on your weakness (**{weakness.replace('_', ' ')}**), try asking about: "
    ...
    for i, suggestion in enumerate(suggested):
        if cols[i % 3].button(suggestion, key=f"suggest_{i}", use_container_width=True):
            st.session_state.rules_chat_history.append({"role": "user", "content": suggestion})
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                if results:
                    entry = results[0]
                    principles = "\n".join(
                        f"- {p}" for p in entry.get("key_principles", [])[:3]
                    )
                    rulebook_content = (
            ...
            try:
                ...
                answer = resp.json().get("response", "Sorry, something went wrong.") if resp.status_code == 200 else "Sorry, try again."
            ...
            st.session_state.rules_chat_history.append({"role": "assistant", "content": answer})
            st.session_state.rules_thinking = False
            st.rerun()
