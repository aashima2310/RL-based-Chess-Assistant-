...
def get_engine_path():
    rl_engine = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "engine", "chessrl_engine"))
    if os.path.exists(rl_engine):
        return rl_engine
    stockfish = shutil.which("stockfish")
    if stockfish:
        return stockfish
    linux_paths = ["/usr/games/stockfish", "/usr/bin/stockfish", "/usr/local/bin/stockfish"]
    ...
    for path in windows_paths:
        if os.path.exists(path):
            return path
    raise FileNotFoundError("Stockfish engine not found. Ensure stockfish is installed in system PATH or /usr/games/stockfish.")
    ...
    try:
        for move in game.mainline_moves():
            ...
            if classification in ("Mistake", "Blunder"):
                ...
                alternatives = [board.san(info["pv"][0]) for info in multi_info if info.get("pv")]
                ...
                try:
                    best_san = board_before_move.san(best_move_raw) if best_move_raw else "N/A"
                except Exception:
