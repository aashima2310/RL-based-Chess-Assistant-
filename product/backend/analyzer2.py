...
def get_engine_path():
    # Project root:
    # analyzer.py is inside product/backend/
    # so ../../ means main project folder
    project_root = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..")
    )

    # First check if RL team's engine exists
    rl_engine = os.path.join(
        project_root,
        "engine",
        "chessrl_engine"
    )
    ...
    # Fall back to Stockfish — auto detect on any OS
    stockfish = shutil.which("stockfish")
    if stockfish:
        return stockfish

    # Check inside project folder first
    project_stockfish_paths = [
        os.path.join(project_root, "stockfish-windows-x86-64-avx2.exe"),
        os.path.join(project_root, "stockfish.exe"),
        os.path.join(project_root, "stockfish-windows-x86-64-avx2"),
        os.path.join(project_root, "stockfish"),
    ]

    for path in project_stockfish_paths:
    ...
    raise FileNotFoundError(
        "Stockfish not found. Put stockfish-windows-x86-64-avx2.exe in the main project folder, "
        "or install Stockfish and add it to PATH."
    )
...
def analyze_pgn(pgn_string):
    ...
    try:
        for move in game.mainline_moves():
            ...
            score_before = (
                info_before["score"].white().score(mate_score=10000) or 0
            )
            ...
            score_after = (
                info_after["score"].white().score(mate_score=10000) or 0
            )
    ...
    mistakes = [
        m for m in moves_analysis
        if m["classification"] == "Mistake"
    ]

    inaccuracies = [
        m for m in moves_analysis
        if m["classification"] == "Inaccuracy"
    ]
